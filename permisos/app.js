...
function muestra(tipo, localidad) {
    ...
    var localidadArray = localidad ? localidad.split(',') : $("input[name='localidad']:checked").map(function() { return this.value; }).get();

    var tipo = tipoArray.join(',');
    var localidad = localidadArray.join(',');
    ...
    $.ajax({
        url: "datos.php?search=" + busca + '&tipo=' + tipo + '&localidad=' + localidad + "&token=" + $('#token').val(),
        type: 'GET',
...
function acerca(){
    ...
    $(document).ready(function() {
        ...
        var lastAcerca = localStorage.getItem('acercaLastShownPermisos');
        var now = Date.now();
        if (!lastAcerca || now - parseInt(lastAcerca, 10) > 3600000) { // 1 hour = 3600000 ms
            //acerca();
            //localStorage.setItem('acercaLastShownPermisos', now.toString());
        }
    });
...
function procesar() {
    ...
    $("#form1").find("input[name='destino[]']:checked").each(function() {
        var valor = $(this).val();
        if (valor) {
    ...
    $("#form1").find("input[name='localidad[]']:checked").each(function() {
        var valor = $(this).val();
        if (valor) {
            localidadArray.push(valor);
        }
    });

    // Unir los arrays en strings separados por comas
    var tipoSelected = tipoArray.join(',');
    var localidadSelected = localidadArray.join(',');
