...
$sql = "SELECT * FROM instrumentos WHERE instrumentos_id=" . mysqli_real_escape_string($db, $_GET['instrumentos_id']);
...
<title><?php echo $_GET["instrumentos_id"] == '0' ? 'Crear Nuevo Instrumento' : 'Editar Instrumento';?></title>
...
function generatePDF() {
    ...
    const opt = {
        margin: 1,
        filename: '<?php echo $row["instrumentos_descripcion"] ?>.pdf',
    ...
    <div id="page-content-wrapper">
        <div class="container-fluid p-4">
            ...
            <div class="row top-info-div">
                <div class="col">
                    <h5><?php echo $_GET["instrumentos_id"] == '0' ? 'CREAR NUEVO INSTRUMENTO' : 'EDITAR INSTRUMENTO';?></h5>
                </div>
                <div class="col text-end">
                    <button onclick="window.location='instrumentos.php'" type="button" class="btn btn-primary"><i class="fa-solid fa-chevron-left"></i>&nbsp;Volver</button>
                </div>
            </div>

            <div id="pprint" class="container">
                <form id="form1" name="form1">
                    <?php if($_GET["instrumentos_id"] != '0'){ ?>
                    <div class="row">
                        <div class="col">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <label for="instrumentos_descripcion">Descripción</label>
                                <input type="text" class="form-control" id="instrumentos_descripcion" name="instrumentos_descripcion" value="<?php echo $row["instrumentos_descripcion"];?>">
                            </div>
                        </div>
                    </div>
                    
                    <input name="instrumentos_id" type="hidden" id="instrumentos_id" value="<?php echo $row["instrumentos_id"]?>" />
                    <input name="posted" type="hidden" id="posted" value="1" />
                    <input name="task" type="hidden" id="task" value="<?php echo $_GET["instrumentos_id"] == '0' ? 'insert' : 'update';?>" />
                    <input name="token" type="hidden" id="token" value="<?php echo $sid;?>" />
                    
                    <div class="row mt-4">
                        <?php if($row_tabla["usuarios_profile"] == 'ADMINISTRADOR' && $_GET["instrumentos_id"] != '0'){ ?>
                        <div class="col text-center">
                            <button id="btdelete" onclick="eliminar();" type="button" class="btn btn-danger"><i class="fa-regular fa-trash-can"></i>&nbsp;Eliminar instrumento</button>
                        </div>
                        <?php } ?>
                        <div class="col text-center">
                ...
                <form id="frm1" name="frm1">
                    ...
                    <input type="hidden" name="instrumentos_id" value="<?php echo $row["instrumentos_id"];?>">
                ...
                $.alert({
                    ...
                    buttons: {
                        confirm: {
                            text: 'Continuar',
                            action: function(){
                                if (response != 0) window.location='instrumentos_editar.php?instrumentos_id='+response;
                            }
                        }
                    }
                });
        ...
        $.ajax({
            ...
            success: function(response) {
                $.confirm({
                    ...
                    buttons: { ok: function () { window.location='instrumentos.php'; } }
                });
            },
            error: function(xhr, status, error) { console.error("Error:", error); },
            beforeSend: function() {
