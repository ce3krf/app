...
$sql = "SELECT * FROM areas WHERE id=" . mysqli_real_escape_string($db, $_GET['id']);
...
<title><?php echo $_GET["id"] == '0' ? 'Crear Nueva Área' : 'Editar Área';?></title>
...
function generatePDF() {
    ...
    const opt = {
        margin: 1,
        filename: '<?php echo $row["area"] ?>.pdf',
    ...
    <div id="page-content-wrapper">
        <div class="container-fluid p-4">
            <div class="row top-info-div">
                <div class="col">
                    <h5><?php echo $_GET["id"] == '0' ? 'CREAR NUEVA ÁREA' : 'EDITAR ÁREA';?></h5>
                </div>
                <div class="col text-end">
                    <button onclick="window.location='areas.php'" type="button" class="btn btn-primary"><i class="fa-solid fa-chevron-left"></i>&nbsp;Volver</button>
                </div>
            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="color" class="form-control" id="color" name="color" value="<?php echo $row["color"] ? $row["color"] : '#f0f0f0';?>" style="height: 50px;">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="color_text">Código de color</label>
                                <input type="text" class="form-control" id="color_text" name="color_text" value="<?php echo $row["color"] ? $row["color"] : '#f0f0f0';?>" readonly style="background-color: <?php echo $row["color"] ? $row["color"] : '#f0f0f0';?>; font-weight: bold;">
                            </div>
                        </div>
                    </div>
                    <input name="id" type="hidden" class="fields30" id="id" value="<?php echo $row["id"]?>" />
                    <input name="posted" type="hidden" class="fields30" id="posted" value="1" />
                    <input name="task" type="hidden" class="fields30" id="task" value="<?php echo $_GET["id"] == '0' ? 'insert' : 'update';?>" />
                    <input name="token" type="hidden" class="fields30" id="token" value="<?php echo $sid;?>" />
                    <div class="row top-info-div">
                        <?php if($row_tabla["usuarios_profile"] == 'ADMINISTRADOR' && $_GET["id"] != '0'){ ?>
                        <div class="col" style="text-align:center">
                            <button id="btdelete" onclick="eliminar();" type="button" class="btn btn-danger"><i class="fa-regular fa-trash-can"></i>&nbsp;Eliminar área</button>
                        </div>
                        <?php } ?>
                        <div class="col" style="text-align:center">
...
$(document).ready(function() {
    ...
    $('#color').on('input', function() {
        ...
        $('#color_text').css('background-color', colorValue);
    });
                ...
                $.alert({
                    ...
                    buttons: {
                        confirm: {
                            ...
                            action: function(){
                                if (response != 0){
                                    window.location='areas_editar.php?id='+response;
                                }
                            }
                        }
                    }
