...
if ($_POST["task"] == "update") {
    ...
    $sql .= "area = '" . mysqli_real_escape_string($db, $_POST['area']) . "', ";
    $sql .= "color = '" . mysqli_real_escape_string($db, $_POST['color']) . "' ";
...
if ($_POST["task"] == "insert") {
    $sql = "INSERT INTO areas (area, color) VALUES ('Nueva área', '#f0f0f0')";
    if(!$result = $db->query($sql)){
        die('Hay un error [' . $db->error . ']');
    }
    
    $sql = "SELECT * FROM areas ORDER BY id DESC LIMIT 1";
    if(!$result = $db->query($sql)){
        die('Hay un error [' . $db->error . ']');
    }
