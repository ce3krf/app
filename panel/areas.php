...
$(document).ready(function() {
    var tabla = $('#tabla').DataTable({
        ...
        columnDefs: [
            ...
            { "targets": 1, visible: true, width: "70%" },
            { "targets": 2, visible: true, width: "20%" }
        ],
        "sAjaxSource": "api/areas.php?token=" + $("#sid").val() + "&task=list",
        "aoColumns": [
            { mData: mix },
            { mData: 'area' },
            { mData: colorCell }
    ...
    function mix(data) {
        return '<a href="areas_editar.php?id=' + data.id + '"><img src="img/document.png" width="32"></a>';
    }
    
    function colorCell(data) {
        if (data.color) {
            return '<div style="display: flex; align-items: center;"><div style="width: 30px; height: 30px; background-color: ' + data.color + '; border: 1px solid #ccc; border-radius: 4px; margin-right: 10px;"></div><span>' + data.color + '</span></div>';
        }
        return '';
    }
});
        ...
        <div class="container-fluid p-4">
            ...
            <div class="row top-info-div">
                ...
                <div class="col text-end">
                    ...
                    <button onclick="window.location='areas_editar.php?id=0'" type="button" class="btn btn-primary"><i class="fa-solid fa-plus"></i>&nbsp;Nueva área</button>
                </div>
            </div>
