...
function muestra(tipo, actividad2) {
    ...
    // Usar los valores de los argumentos tipo y localidad si están definidos, si no, obtenerlos de los checkboxes
    var tipoArray = tipo ? tipo.split(',') : $("input[name='tipo']:checked").map(function() { return this.value; }).get();
    var actividad2Array = actividad2 ? actividad2.split(',') : $("input[name='actividad2']:checked").map(function() { return this.value; }).get();

    var tipo = tipoArray.join(',');
    var actividad2 = actividad2Array.join(',');
    //console.log(tipo);
    //console.log(area);
    ...
    howmany = 0;
    $('#m1').html('<span style="color:gold;">&#9733; </span> 0 patentes');

    $.ajax({
        url: "datos.php?search=" + busca + '&tipo=' + tipo + '&actividad2=' + actividad2 + '&token=' + $('#token').val(),
        type: 'GET',
        success: function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.aaData.length; i++) {
                    ...
                    howmany++;
                    $('#m1').html('<span style="color:gold;">&#9733; </span>' + howmany + ' patentes');
...
function agregar(arr) {
    ...
    var marker = L.circleMarker([arr['LATITUD'], arr['LONGITUD']], {
        color: strk,
        fillColor: clr,
        weight: 2,
        fillOpacity: 0.8,
    ...
    var text2 = L.tooltip({
        ...
        className: 'etiqueta',
        permanent: true
    }).setLatLng([arr['LATITUD'], arr['LONGITUD']]);
    marker.bindTooltip(text2);

    tooltip = "<b>" + arr['DIRECCION_COMERCIAL'] + "</b><br>";
    tooltip += "Tipo: " + arr['TIPO_PATENTE'] + "<br>";
    tooltip += "Giro: " + arr['GIRO'] + "<br>";
    tooltip += "Actividad: " + arr['actividad2'] + "<br>";
...
function procesar() {
    ...
    // Recorre los checkboxes de tipo (destino)
    $("#form1").find("input[name='tipo[]']:checked").each(function() {
        var valor = $(this).val();
        if (valor) {
            tipoArray.push(valor);
        }
    });

    // Recorre los checkboxes de localidad
    $("#form1").find("input[name='actividad2[]']:checked").each(function() {
        var valor = $(this).val();
        if (valor) {
            actividad2Array.push(valor);
        }
    });

    // Unir los arrays en strings separados por comas
    var tipoSelected = tipoArray.join(',');
    var actividad2Selected = actividad2Array.join(',');

    //limpiar();
    tipo = tipoSelected;
    actividad2 = actividad2Selected;
    //console.log('tipo:', tipo);
    //console.log('localidad:', localidad);

    // Llamar a la función muestra con los valores procesados
    muestra(tipo, actividad2);
...
function acerca(){
    ...
    $('#acerca').on('click', function () {
        ...
        $(document).ready(function() {
        // Show "acerca" if more than one hour has passed since last time
        var lastAcerca = localStorage.getItem('acercaLastShownPatentes');
        var now = Date.now();
        if (!lastAcerca || now - parseInt(lastAcerca, 10) > 3600000) { // 1 hour = 3600000 ms
            //acerca();
            //localStorage.setItem('acercaLastShownPatentes', now.toString());
        }
    });